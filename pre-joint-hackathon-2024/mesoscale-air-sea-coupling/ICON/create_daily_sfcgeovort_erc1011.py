...
expid='erc1011'
outdir='/work/mh0287/m300466/EERIE/'+expid+'/geostrohpic/'
...
for ii in range(0,len(fdatearr)):
    ...
    nrelvort=np.where(np.abs(g0*fCo)<=2*omega*np.sin(Colatlim/180*np.pi),fillval,nrelvort)*lsmask2
    ...
    relvortfile=outdir+'relvort/dm/'+expid+'_relvort_dm_'+fdate+'.nc'
    print('Write to '+relvortfile)
    writenc1d_r2b9O(relvortfile,nctime,14886338,sfcvort,'relvort','Surface geostrophic relative vorticity','rel_vort','1/s')
